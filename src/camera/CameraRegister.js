...
export default class CameraRegister extends React.Component {
  ...
  constructor(props) {
    ...
    Alert.alert(
      "Manual Information",
      "1. Your face should be place within the Box \n" +
        "2. Then the face register will start \n" +
        "3. Keep your face within the Box during the register ",
  ...
  afterFaceDetected = ({ faces }) => {
    ...
    if (state.saveNumOfFace < 10 && state.processing) {
      ...
      this.camera.takePictureAsync().then(data => {
        ...
        imageForm.append();
        fetch("http://192.168.0.61:3000/uploadfile", {
          method: "POST",
          body: imageForm
        })
    ...
    } else if (state.saveNumOfFace == 10) {
      this.setState({ canDetectFaces: false });
      this.setState({ loading: false });
      alert("Now you can move your face. Now the server work begins.")
    }
  };
  ...
  modelBuild = () => {
    ...
    if (!state.processEnd) {
      ...
      this.setState({ loading: true });
      fetch("http://192.168.0.61:3000/build", {
        method: "POST",
        ...
        .then(response => response.json())
        .then(responseData => {
          if (responseData.response == "model build") {
            this.setState({ loading: false });
    ...
    return (
      ...
      >
        ...
        <Text style={styles.faceText}>
          Complete: {state.saveNumOfFace} / 10
        </Text>
      </View>
    );
  }
